...
class ModelUserProviderBuilder implements UserProviderBuilderInterface
{
    public function create(array $options): UserProviderInterface
    {
        if (! isset($options['class'])) {
            throw new LogicException("The 'class' option must be configured for the model user provider.");
        }

        if (! isset($options['identifier'])) {
            throw new LogicException("The 'identifier' option must be configured for the model user provider.");
        }
